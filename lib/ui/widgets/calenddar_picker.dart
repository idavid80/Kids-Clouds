...
class _DateRangePickerPopupState extends State<DateRangePickerPopup> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Selecciona rango de fechas'),
      content: SizedBox(
        width: 300,
        height: 300,
        child: TableCalendar(
          ...
          focusedDay: _tempRange.start,
          calendarFormat: CalendarFormat.month,
          selectedDayPredicate: (day) {
            return (day.isAtSameMomentAs(_tempRange.start) ||
                day.isAtSameMomentAs(_tempRange.end) ||
                (day.isAfter(_tempRange.start) && day.isBefore(_tempRange.end)));
          },
          onDaySelected: (selectedDay, focusedDay) {
            setState(() {
              if (_tempRange.start == _tempRange.end) {
                if (selectedDay.isBefore(_tempRange.start)) {
                  _tempRange = DateTimeRange(start: selectedDay, end: _tempRange.start);
                } else {
                  _tempRange = DateTimeRange(start: _tempRange.start, end: selectedDay);
                }
              } else {
                _tempRange = DateTimeRange(start: selectedDay, end: selectedDay);
              }
            });
          },
        ),
